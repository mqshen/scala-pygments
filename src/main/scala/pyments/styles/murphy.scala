package pyments.styles

import java.lang.Number

import pyments._
import pyments._

class MurphyStyle extends Style {
  """
    Murphy's style from CodeRay.
  """

  val default_style = ""

  var styles = collection.mutable.Map[Token, String](
    Whitespace -> "#bbbbbb",
    Comment -> "#666 italic",
    Comment.Preproc -> "#579 noitalic",
    Comment.Special -> "#c00 bold",

    Keyword -> "bold #289",
    Keyword.Pseudo -> "#08f",
    Keyword.Type -> "#66f",

    Operator -> "#333",
    Operator.Word -> "bold #000",

    Name.Builtin -> "#072",
    Name.Function -> "bold #5ed",
    Name.Class -> "bold #e9e",
    Name.Namespace -> "bold #0e84b5",
    Name.Exception -> "bold #F00",
    Name.Variable -> "#036",
    Name.Variable.Instance -> "#aaf",
    Name.Variable.Class -> "#ccf",
    Name.Variable.Global -> "#f84",
    Name.Constant -> "bold #5ed",
    Name.Label -> "bold #970",
    Name.Entity -> "#800",
    Name.Attribute -> "#007",
    Name.Tag -> "#070",
    Name.Decorator -> "bold #555",

    Str -> "bg:#e0e0ff",
    Str.Char -> "#88F bg:",
    Str.Doc -> "#D42 bg:",
    Str.Interpol -> "bg:#eee",
    Str.Escape -> "bold #666",
    Str.Regex -> "bg:#e0e0ff #000",
    Str.Symbol -> "#fc8 bg:",
    Str.Other -> "#f88",

    pyments.Number -> "bold #60E",
    pyments.Number.Integer -> "bold #66f",
    pyments.Number.Float -> "bold #60E",
    pyments.Number.Hex -> "bold #058",
    pyments.Number.Oct -> "bold #40E",

    Generic.Heading -> "bold #000080",
    Generic.Subheading -> "bold #800080",
    Generic.Deleted -> "#A00000",
    Generic.Inserted -> "#00A000",
    Generic.Error -> "#FF0000",
    Generic.Emph -> "italic",
    Generic.Strong -> "bold",
    Generic.Prompt -> "bold #c65d09",
    Generic.Output -> "#888",
    Generic.Traceback -> "#04D",

    Error -> "#F00 bg:#FAA")

}
